...
void check_digital_encoder(CO_Data *d, UNS8 nodeid)
{
    ...
    if (getReadResultNetworkDict(d, nodeid, &data, &size, &abortCode) != SDO_FINISHED)
    {
        printf("\nRead Digital Encoder Result : Failed in getting information for slave %2.2x, AbortCode :%4.4x \n", nodeid, abortCode);
    }
    else
    {
        printf("\nNode ID: %d Digital Encoder Value : %d\n", nodeid, data);
        g_epos_encoder_value[nodeid] = (UNS32) data;
    }
    /* Finalize last SDO transfer with this node */
    closeSDOtransfer(d, nodeid, SDO_CLIENT);
}
...
void check_vel_actual_value(CO_Data *d, UNS8 nodeid)
{
    ...
    {
        printf("\nNode ID: %d Velocity Actual Value : %d\n", nodeid, data);
        g_epos_actul_speed[nodeid] = (UNS32) data;
    }
    /* Finalize last SDO transfer with this node */
    closeSDOtransfer(d, nodeid, SDO_CLIENT);
}
...
UNS8 read_vel_actual_value(UNS8 node_id)
{
    ...
    UNS8  datatype = 0;
    UNS8  read_ret = readNetworkDictCallback(&MasterOD_Data, (UNS8) node_id, (UNS16) index, (UNS8) subindex,
                                             (UNS8) datatype, check_vel_actual_value, 0);
    if (read_ret == 0)
    {
        printf("Send read sdo successfully.\n");
    } else if (read_ret == 0xFE)
...
UNS8 set_pvm_velocity(UNS8 node_id, INTEGER32 vel)
{
    ...
    /*
    data[0] = (UNS8) vel;
    data[1] = (UNS8) ( vel >> sizeof(UNS8) );
    data[2] = (UNS8) ( vel >> sizeof(UNS8) * 2 );
    data[3] = (UNS8) ( vel >> sizeof(UNS8) * 3 );
    */
    memcpy((void*)data, (void*)&vel, sizeof(UNS8) * 4);
...
void sdo_callback(CO_Data *d, UNS8 nodeid)
{
    ...
    {
        //set sdo read data
        pthread_mutex_lock(&g_epos_sync_read_sdo_mutex);
        g_epos_sync_read_sdo_data = data;
        pthread_mutex_unlock(&g_epos_sync_read_sdo_mutex);
        //notify event
        pthread_cond_signal(&g_epos_sync_read_sdo_cond);
    }
    /* Finalize last SDO transfer with this node */
    closeSDOtransfer(d, nodeid, SDO_CLIENT);
}
...
{
    UNS8  datatype = 0;
    UNS8  read_ret = readNetworkDictCallback(&MasterOD_Data, (UNS8) node_id, (UNS16) index, (UNS8) sub_index,
                                            (UNS8) datatype, sdo_callback, 0);
    if (read_ret == 0)
    {
        printf("Send read sdo successfully.\n");
    }
